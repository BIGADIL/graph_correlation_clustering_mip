...
class MDAMIPIneqModel(MDAAbstractModel):
    """
    MinDisAgree MIP model which uses x_ij + x_ir + x_jr != 1 constraint.
    """
    ...
    def __init__(self,
        ...
        :param solver_name: solver which solves problem. Default: CBC.
        :param threads: number of model threads. Default -1, i.e. model uses all available threads.
        """
    ...
    def add_variables(self) -> None:
        ...
        for i in range(self.size):
            col = []
            for j in range(i):
                row = [self.model.add_var(var_type=BINARY) for _ in range(j)]
                col.append(row)
            self.y.append(col)
    ...
    def add_constraints(self) -> None:
        for i in range(self.size):
            for j in range(i):
                for k in range(j):
                    self.model += self.x[i][j] + self.x[i][k] + self.x[j][k] >= 1 + self.eps - (
                                1 - self.y[i][j][k]) * self.M
                    self.model += self.x[i][j] + self.x[i][k] + self.x[j][k] <= 1 - self.eps + self.y[i][j][k] * self.M
...
class MDAMIPIneqModelLeqK(MDAMIPIneqModel):
    def __init__(self,
                 graph: List[List[int]],
                 k: int,
                 solver_name: str = "cbc",
    ...
    def add_constraints_for_k_3(self):
        for i in range(self.size):
            for j in range(i):
                for k in range(j):
                    for r in range(k):
                        self.model += self.x[i][j] + self.x[i][k] + self.x[i][r] + self.x[j][k] + self.x[j][r] + \
                                      self.x[k][r] <= 5
