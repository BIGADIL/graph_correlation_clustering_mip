...
class MDAORModuleModel(MDAAbstractModel):
    def __init__(self,
                 graph: List[List[int]],
                 k: int = None) -> None:
    ...
    def add_variables_for_k_2(self) -> None:
        ...
        for i in range(1, self.size):
            self.x.append(self.model.BoolVar("x_" + str(i)))
        for i in range(self.size):
            ...
            for j in range(i):
                if j == 0:
                    ...
                    row_u.append(self.model.BoolVar("u_" + str(i) + str(j)))
                    row_v.append(self.model.BoolVar("v_" + str(i) + str(j)))
            self.u.append(row_u)
            self.v.append(row_v)
    ...
    def add_variables_for_k_geq_3(self):
        ...
        for i in range(1, self.size):
            ...
            for j in range(self.k):
                row_x.append(self.model.BoolVar("x_" + str(i) + str(j)))
            self.x.append(row_x)
        for i in range(self.size):
            row_u = []
            row_v = []
            for j in range(i):
                if j == 0:
                    ...
                    for r in range(self.k):
                        col_u.append(self.model.BoolVar("u_" + str(i) + str(j) + str(r)))
                        col_v.append(self.model.BoolVar("v_" + str(i) + str(j) + str(r)))
                row_u.append(col_u)
                row_v.append(col_v)
            self.u.append(row_u)
            self.v.append(row_v)
    ...
    def add_goal_for_k_2(self):
        ...
        self.pos_part = self.model.Sum(x_neg)
        self.neg_part = self.model.Sum(x_pos)
        ...
        for i in range(self.size):
            for j in range(i):
                if j == 0:
                    ...
                    if self.graph[i][j] == 1:
                        x_pos.append(0.5 * self.model.Sum(buffer))
                    else:
                        x_neg.append(0.5 * self.model.Sum(buffer))
                        const += 0.5 * (self.k - 2)
        self.pos_part = self.model.Sum(x_neg)
        self.neg_part = self.model.Sum(x_pos)
        self.model.Minimize(self.pos_part + self.neg_part - const)
        ...
        for i in range(self.size):
            for j in range(i):
                if j == 0:
                    continue
                if self.graph[i][j] == 1:
                    self.model.Add(self.x[i] - self.x[j] + self.u[i][j] - self.v[i][j] == 0)
                else:
                    self.model.Add(self.x[i] + self.x[j] - 1 + self.u[i][j] - self.v[i][j] == 0)

    def add_constraints_for_k_geq_3(self):
        for i in range(1, self.size):
            self.model.Add(self.model.Sum(self.x[i]) == 1)
            for j in range(1, i):
                for r in range(self.k):
                    if self.graph[i][j] == 1:
                        self.model.Add(self.x[i][r] - self.x[j][r] + self.u[i][j][r] - self.v[i][j][r] == 0)
                    else:
                        self.model.Add(self.x[i][r] + self.x[j][r] - 1 + self.u[i][j][r] - self.v[i][j][r] == 0)
    ...
    def get_clustering_vector_for_k_2(self):
        ...
        res[0] = 0
        for i in range(1, self.size):
            res[i] = int(self.x[i].solution_value())
        return res
    ...
    def get_clustering_vector_for_k_geq_3(self):
        ...
        for i in range(1, self.size):
            for r in range(self.k):
                if int(self.x[i][r].solution_value()) == 1:
                    if r in d:
                        d[r].append(i)
                    else:
                        d[r] = [i]
